...
-- This ensures advisors see ONLY their own data without policies blocking each other
...
CREATE POLICY "Portal can view client via case" ON public.clients
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM cases WHERE cases.client_id = clients.id AND cases.portal_enabled = true)
  );
...
CREATE POLICY "Advisors can view case documents" ON public.case_documents
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM cases WHERE cases.id = case_documents.case_id AND cases.advisor_id = auth.uid())
  );
...
CREATE POLICY "Portal access to documents" ON public.case_documents
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM cases WHERE cases.id = case_documents.case_id AND cases.portal_enabled = true)
  );
...
CREATE POLICY "Advisors can view uploads" ON public.uploads
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM cases WHERE cases.id = uploads.case_id AND cases.advisor_id = auth.uid())
  );
...
CREATE POLICY "Portal can view uploads" ON public.uploads
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM cases WHERE cases.id = uploads.case_id AND cases.portal_enabled = true)
  );
...
CREATE POLICY "Portal can delete uploads" ON public.uploads
  FOR DELETE USING (
    EXISTS (SELECT 1 FROM cases WHERE cases.id = uploads.case_id AND cases.portal_enabled = true)
...
CREATE POLICY "Advisors can view own notifications" ON public.notifications
  FOR SELECT USING (auth.uid() = advisor_id);

CREATE POLICY "Advisors can update own notifications" ON public.notifications
  FOR UPDATE USING (auth.uid() = advisor_id);

CREATE POLICY "Advisors can delete own notifications" ON public.notifications
  FOR DELETE USING (auth.uid() = advisor_id);
...
CREATE POLICY "Advisors can view activity logs for their cases" ON public.case_activity_log
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM cases WHERE cases.id = case_activity_log.case_id AND cases.advisor_id = auth.uid())
  );

CREATE POLICY "Allow inserts for case activity" ON public.case_activity_log
  FOR INSERT WITH CHECK (true);

CREATE POLICY "Portal users can view activity logs" ON public.case_activity_log
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM cases WHERE cases.id = case_activity_log.case_id AND cases.portal_enabled = true)
  );
